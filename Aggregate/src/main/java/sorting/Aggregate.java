...
public class Aggregate {
    ...
    public static int[][] aggregate(int[][] input, int column) {
        Arrays.sort(input, Comparator.comparingInt(a -> a[column]));
        ...
        for (int i = 1; i <= n; i++) {
            if (i == n || input[i][column] != input[start][column]) {
                int[] aggregatedRow = new int[m];
                for (int j = 0; j < m; j++) {
                    aggregatedRow[j] = mode(input, start, i - 1, j);
                }
